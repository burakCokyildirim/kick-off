...
public func render(_ nodes: [Node], voidElements: Set<String> = voidElements) -> String {
...
public func render(_ node: Node, voidElements: Set<String> = voidElements) -> String {
    var rendered = ""
    render(node, into: &rendered, voidElements: voidElements)
    return rendered
}

public func render<T>(_ children: [ChildOf<T>], voidElements: Set<String> = voidElements) -> String {
    return children.reduce(into: "") { $0.append(render($1, voidElements: voidElements)) }
}

public func render<T>(_ child: ChildOf<T>, voidElements: Set<String> = voidElements) -> String {
...
func render(_ node: Node, into output: inout String, voidElements: Set<String>) {
    ...
    case let .doctype(string):
        ...
        output.append(string)//escapeDoctype(string))
        output.append(">")
    ...
    case let .text(string):
        output.append(string)//escapeTextNode(text: ))
    case let .raw(string):
